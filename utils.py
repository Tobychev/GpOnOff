...
def setup_makers(config, src_pos, extra_conf):
    ...
    scaffold = gds.SpectrumDataset.create(geom=geom, energy_axis_true=e_true)
...
def make_dataset(observations, ana_conf, src_pos, conf):
    # ## Setup makers
    bkg_maker, safe_mask_maker, dm_maker, scaffold = setup_makers(
    ...
    for ob in observations:
        dataset = dm_maker.run(scaffold.copy(name=str(ob.obs_id)), ob)
...
def make_fluxpoints(analysis, conf):
    ...
    energy_edges = (
        np.logspace(
            np.log10(analysis.config.fit.fit_range.min.value),
...
def resample_dataset(spectrum, conf, rebin):
    fit_axis = gmap.MapAxis.from_energy_bounds(
        conf["fit_min"], conf["fit_max"], nbin=rebin, unit="TeV", name="energy"
    )
    ...
    # Note the plural-s
        ...
        raise NotImplementedError(
            f"Not able to resample spectrum of type {type(spectrum)}"
        )
...
def save_fit_result(fit_result, ana_conf, conf, E_thr):
    ...
    fit_result.add_row(
        [
            "",
            "min_energy",
            ana_conf.config.fit.fit_range.min,
            "TeV",
            0.0,
            None,
            None,
    ...
    fit_result.add_row(
        [
            "",
            "max_energy",
            ana_conf.config.fit.fit_range.max,
            "TeV",
            0.0,
            None,
            None,
...
def quick_print_stats(stats):
    print(stats[["excess", "sqrt_ts", "ontime", "counts_rate", "background_rate"]][-1])
...
def calc_analytical_decorr(analysis):
    ...
    """
    covar = analysis.models.covariance.get_subcovariance(
        ["index", "amplitude", "reference"]
    ).data
    e_decorr = np.exp(
        covar[0, 1] / covar[0, 0] / analysis.models.parameters["amplitude"].value
    )
